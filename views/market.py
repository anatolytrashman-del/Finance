...
with page(
    ...
    "Источники: kufar.by, realt.by · " + " · ".join(q["name"] for q in MONITORED_QUARTALS),
):
    ...
    section_title("🏗️ Цены застройщика (bir.by)")
    st.caption(
    ...
    if not bir_cache or not bir_cache.get("listings"):
        ...
        for _bq_i, quartal in enumerate(BIR_QUARTALS):
            q_df = bir_df[bir_df["quartal"] == quartal]
            with card("market", f"bir_{_bq_i}"):
                section_title(f"🏠 {quartal}")
                if q_df.empty:
    ...
    st.divider()
    section_title("📊 Вторичный рынок (kufar.by, realt.by)")
    ...
    for _sq_i, quartal in enumerate(MONITORED_QUARTALS):
        q_name = quartal["name"]
        q_labels = [a["label"] for a in quartal["addresses"]]
        sub = df[df["address"].isin(q_labels)]
        with card("market", f"sec_{_sq_i}"):
            section_title(f"🏠 {q_name}")
