...
from theme import DANGER, GOOD, NEUTRAL, card, page, pill, section_title
...
with page("ideas", "💡", "Инвест-идеи", "Что рассматриваем, что реализовали, от чего отказались"):
    # --- Форма добавления ---
    ...
    if not active:
        ...
        for idea in active:
            with card("ideas", f"active_{idea['id']}"):
                if editing_id == idea["id"]:
                    ...
                    if idea.get("status"):
                        head.markdown(pill(idea["status"], NEUTRAL), unsafe_allow_html=True)
    ...
    if done:
        st.divider()
        section_title(f"✅ Реализованные идеи ({len(done)})")
        done_sorted = sorted(done, key=lambda i: i.get("done_at", ""), reverse=True)
        for idea in done_sorted:
            with card("ideas", f"done_{idea['id']}"):
                head, back_btn, del_btn = st.columns([8, 1, 1])
                head.markdown(f"### {_md(idea['name'])}")
                if idea.get("done_at"):
                    head.markdown(pill(f"Реализовано {_fmt_date(idea['done_at'])}", GOOD), unsafe_allow_html=True)
                if idea.get("status"):
    ...
    if rejected:
        st.divider()
        section_title(f"⛔ Отклонённые идеи ({len(rejected)})")
        rejected_sorted = sorted(rejected, key=lambda i: i.get("rejected_at", ""), reverse=True)
        for idea in rejected_sorted:
            with card("ideas", f"rejected_{idea['id']}"):
                head, back_btn, del_btn = st.columns([8, 1, 1])
                head.markdown(f"### {_md(idea['name'])}")
                if idea.get("rejected_at"):
                    head.markdown(pill(f"Отклонено {_fmt_date(idea['rejected_at'])}", DANGER), unsafe_allow_html=True)
                if idea.get("status"):
